...
if (typeof process.argv[2] === 'string' && process.argv[2] !== '') {
	[
		[`${process.argv[2]}.png`, 'logo.png'],
		[`${process.argv[2]}.svg`, 'logo.svg']
	].forEach((elements) => {
		fs.copyFile(`static\\logo\\${elements[0]}`, `static\\logo\\${elements[1]}`, (err) => {
			if (err) {
				console.log(`Can't copy static\\logo\\${elements[0]} to static\\logo\\${elements[1]}`);
				console.error(err);
				process.exit(1);
			}
		});
